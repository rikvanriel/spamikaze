...
sub main {
	...
	foreach $ip ($Spamikaze::db->get_listed_addresses()) {
		if (Spamikaze::IsIPv6($ip)) {
			# IPv6: nibble-reverse the /64 prefix, prepend wildcard
			my $norm = Spamikaze::NormalizeIPv6($ip);
			next unless defined $norm;
			# Extract first 4 groups (the /64 prefix portion)
			my @groups = split(/:/, $norm);
			my $prefix_hex = join('', @groups[0..3]);
			my $reversed = join('.', reverse split(//, $prefix_hex));
			print ZONEFILE "*.$reversed\t$ttl\tIN\tA\t127.0.0.2\n";
			print ZONEFILE "\t\t$ttl\tIN\tTXT\t\"$dnsbl_url_base$ip\"\n";
		} else {

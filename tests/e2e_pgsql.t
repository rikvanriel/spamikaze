#!/usr/bin/perl
#
# End-to-end tests using a real PostgreSQL backend.
# Automatically starts/stops a temporary PostgreSQL instance.
# Skips if initdb is not available (install postgresql-server).
#
...
BEGIN {
    my $socket_dir = "$FindBin::Bin/db/pgsql/socket";

    # Check for required Perl modules
    for my $mod (qw(DBD::Pg Config::IniFiles Try::Tiny)) {
        eval "require $mod";
        if ($@) {
            require Test::More;
    ...
    # --- Create config pointing to test PostgreSQL ---
...
# --- Load real Spamikaze (uses test config + real PgSQL_3) ---

use lib "$FindBin::Bin/../scripts";
use Spamikaze;
use Test::More;

my $SETUP_SCRIPT = "$FindBin::Bin/db/setup-pgsql.sh";
my $SOCKET_FILE  = "$FindBin::Bin/db/pgsql/socket/.s.PGSQL.5432";
my $we_started_pg = 0;

# Start PostgreSQL if not already running
unless (-e $SOCKET_FILE) {
    ...
    if ($? != 0) {
        ...
        if ($? != 0) {
            plan skip_all => 'Could not start PostgreSQL test instance'
...
# Verify database connectivity
{
    ...
    unless ($dbh) {
        plan skip_all => 'Cannot connect to test PostgreSQL: '
            . ($@ || 'unknown error');
    }
    $dbh->disconnect();
}
...
sub get_blocklist_ip {
    my ($ip) = @_;
    my $dbh = Spamikaze::DBConnect();
    my $sth = $dbh->prepare("SELECT ip, expires FROM blocklist WHERE ip = ?::inet");
    $sth->execute($ip);
...
sub get_emails_for_ip {
    ...
    my $dbh = Spamikaze::DBConnect();
    my $sth = $dbh->prepare(
        'SELECT ip, spam, email FROM emails WHERE ip = ?::inet ORDER BY "time"');
    $sth->execute($ip);
    my @rows;
...
sub set_expiry_past {
    my ($ip) = @_;
    my $dbh = Spamikaze::DBConnect();
    $dbh->do("UPDATE blocklist SET expires = CURRENT_TIMESTAMP - interval '1 hour'"
           . " WHERE ip = ?::inet", undef, $ip);
    $dbh->commit();
    $dbh->disconnect();
}
...

{
    ...
    like($row->{ip}, qr/198\.51\.100\.42/, 'storeip: correct IP stored');
...

{
    ...
    # Store same IP again — upsert rule should UPDATE, not duplicate
    sleep(1);
    $Spamikaze::db->storeip('198.51.100.42', 'received spamtrap mail');
    ...
    my $dbh = Spamikaze::DBConnect();
    my $sth = $dbh->prepare(
        "SELECT COUNT(*) FROM ipevents WHERE ip = ?::inet");
    $sth->execute('198.51.100.42');

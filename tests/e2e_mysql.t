...
# ===== IPv6: storeip stores /64 prefix in blocklist, normalized full IP in ipevents =====

{
    ...
    # The /64 prefix (fully expanded) should be in blocklist
    my $row = get_blocklist_ip('2001:0db8:1234:5678:0000:0000:0000:0000');
    ok(defined $row, 'IPv6 storeip: /64 prefix is in blocklist');

    # The normalized full IP should be in ipevents
    my $dbh = Spamikaze::DBConnect();
    my $sth = $dbh->prepare(
        "SELECT e.eventtext FROM ipevents i"
      . " JOIN eventtypes e ON i.eventid = e.id"
      . " WHERE i.ip = ?");
    $sth->execute('2001:0db8:1234:5678:0000:0000:0000:0001');
    my ($eventtext) = $sth->fetchrow_array();
    $sth->finish();
    $dbh->disconnect();
    is($eventtext, 'received spamtrap mail', 'IPv6 storeip: normalized full IP in ipevents');
}
...

{
    ...
    # Event values should include the individual IPs (normalized)
    my @vals = values %iplog;
    ok(grep(/2001:0db8:1234:5678:0000:0000:0000:0001/, @vals),
        'IPv6 get_listing_info: first IP in events');
    ok(grep(/2001:0db8:1234:5678:0000:0000:0000:ffff/, @vals),
...

{
    ...
    # Remove using a different IP in the same /64
...

{
    ...
    # Normalized full IP in ipevents (via get_listing_info)
    my ($listed, %iplog) = $Spamikaze::db->get_listing_info('2001:db8:abcd:ef01::42');
    is($listed, 1, 'IPv6 process_mail: listed');
    my @vals = values %iplog;
    ok(grep(/2001:0db8:abcd:ef01:0000:0000:0000:0042/, @vals),
        'IPv6 process_mail: normalized full IP in events');
}
